...
private:
    void _drawList();
    void _update(const int delta, const bool wrap = false);
    void _updateHighlight(const int oldCommand, const int newCommand);
    ...
    int _currentCommand;
    int _bottomIndex; // number of command displayed on last line of popup
