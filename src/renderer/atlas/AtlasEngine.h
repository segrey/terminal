...
#pragma warning(suppress : 26490) // Don't use reinterpret_cast (type.1).
        ...
        struct BoringHashset
        {
            AtlasPair& emplace(AtlasKey&& key, bool& inserted)
            {
                ...
                {
                    Buffer<AtlasPair> newEntries{ initialized, entries.size() << 1 };
                    const auto newMask = (mask << 1) | 1;

                    auto it = entries.data();
                    const auto end = it + entries.size();

                    for (; it != end; ++it)
                    {
                        for (uint32_t i = 0;; ++i)
                        {
                            const auto target = &newEntries[(it->hash + i) & newMask];
                ...
                for (uint32_t i = 0;; ++i)
                {
                    const auto it = &entries[(hash + i) & mask];
        ...
        private:
            Buffer<AtlasPair> entries{ initialized, 64 };
            size_t size = 0;
            u32 mask = 63;
        };
