...
class UiaTextRangeTests
{
    ...
    TEST_METHOD(ExpandToEnclosingUnit)
    {
        ...
        auto verifyExpansion = [&](TextUnit textUnit, til::point utrStart, const til::point utrEnd) {
    ...
    TEST_METHOD(MovementAtExclusiveEnd)
    {
        ...
        // GH#6986: This is used as the "end of the buffer" to help screen readers run faster
        //          instead of parsing through thousands of empty lines of text.
        const til::point documentEndInclusive{ base::ClampSub(bufferSize.right, 1), _pTextBuffer->GetLastNonSpaceCharacter().y };
        const til::point documentEndExclusive{ bufferSize.left, base::ClampAdd(documentEndInclusive.y, 1) };
    ...
    TEST_METHOD(ScrollIntoView)
    {
        ...
        const std::vector<ScrollTest> testData{
            ...
            { L"ViewportHeight From Top - 1", base::MakeClampedNum(bufferSize.top) + viewportSize.Height() - 1 },
            { L"ViewportHeight From Top", base::MakeClampedNum(bufferSize.top) + viewportSize.Height() },
            { L"ViewportHeight From Top + 1", base::MakeClampedNum(bufferSize.top) + viewportSize.Height() + 1 },
            { L"ViewportHeight From Bottom - 1", base::MakeClampedNum(bufferSize.bottom) - viewportSize.Height() - 2 },
            { L"ViewportHeight From Bottom", base::MakeClampedNum(bufferSize.bottom) - viewportSize.Height() - 1 },
            { L"ViewportHeight From Bottom + 1", base::MakeClampedNum(bufferSize.bottom) - viewportSize.Height() + 1 },
