...
{
    return FromInclusive(til::inclusive_rect{ sr.left, sr.top, sr.right - 1, sr.bottom - 1 });
}
...
{
    return FromExclusive(til::rect{ origin.x, origin.y, origin.x + width, origin.y + height });
}
...
{
    return FromExclusive(til::rect{ origin.x, origin.y, origin.x + dimensions.width, origin.y + dimensions.height });
}
...
bool Viewport::MoveInBounds(const ptrdiff_t move, til::point& pos) const noexcept
{
...
bool Viewport::TrimToViewport(_Inout_ til::rect* const psr) const noexcept
{
...
// Return Value:
// - <none>
void Viewport::ConvertToOrigin(_Inout_ til::rect* const psr) const noexcept
{
    const auto dx = Left();
    const auto dy = Top();
    psr->left -= dx;
